...
#define R_VEC_TYPE(vec_type, type) \
	...
	static inline R_MAYBE_UNUSED R_MUSTUSE vec_type *R_VEC_FUNC(vec_type, clone)(const vec_type *vec) { \
		...
		void *buf = malloc (sizeof (vec_type ## Meta) + capacity * sizeof (type)); \
		if (R_LIKELY (buf)) { \
	...
	static inline R_MAYBE_UNUSED bool R_VEC_FUNC(vec_type, reserve)(vec_type *vec, ut64 new_capacity) { \
		r_return_val_if_fail (vec && vec->_start, false); \
		vec_type ## Meta *meta = ((void*) vec->_start) - sizeof (vec_type ## Meta); \
		const ut64 capacity = meta->capacity; \
		if (new_capacity > capacity) { \
			vec_type ## Meta *new_meta = realloc (meta, sizeof (vec_type ## Meta) + new_capacity * sizeof (type)); \
			const bool is_success = new_meta != NULL; \
	...
	static inline R_MAYBE_UNUSED void R_VEC_FUNC(vec_type, shrink_to_fit)(vec_type *vec) { \
		...
		if (num_elems != capacity) { \
			void *old_buf = ((void*) vec->_start) - sizeof (vec_type ## Meta); \
			void *buf = realloc (old_buf, sizeof (vec_type ## Meta) + num_elems * sizeof (type)); \
			if (R_LIKELY (buf)) { \
				vec_type ## Meta *meta = buf; \
				meta->capacity = num_elems; \
				vec->_start = buf + sizeof (vec_type ## Meta); \
				vec->_end = vec->_start + num_elems; \
			} \
		} \
	} \
